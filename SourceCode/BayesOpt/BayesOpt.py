...
class BO(object):
    ...
    def __init__(self, search_space, obj_func, surrogate, ftarget=None,
        ...
        ----------
            ...
            init_sol_points: list of lists, initial points to evaluate
        """
        ...
        self.init_sol_points = init_sol_points
        if self.init_sol_points is not None:
            if isinstance(self.init_sol_points[0], list):
                self.n_init_sample -= len(self.init_sol_points)
            else:
                self.n_init_sample -= 1
    ...
    def _initialize(self):
        ...
        while True:
            ...
            if self.init_sol_points is not None:
                if isinstance(self.init_sol_points[0], list):
                    for x in self.init_sol_points:
                        DOE.append(x)
                else:
                    DOE.append(self.init_sol_points)
    ...
    def _argmax_multistart(self, obj_func):
        ...
        for iteration in range(self._random_start):
            ...
            if self._optimizer == 'BFGS':
                ...
                if(type(fopt_)== 'list' or type(fopt_) == 'numpy.ndarray' ):
                    fopt_ = -fopt.item()
                else:
                    fopt_ = -fopt_
